...
//import com.bsnnms.bean.solr.SolrCoreTaskLog;
//import com.bsnnms.spring.config.JSONUtils;
...
//    public static boolean sendHttpPost(String completeUrl, String coreName, SolrCoreTaskLog taskLog, boolean bEnableLog) {
...
//            conn.connect();
//            if(bEnableLog) log.info(core + "<index update process> Request method\t\t\t" + conn.getRequestMethod());
//            if(bEnableLog) log.info(core + "<index update process> Full URL\t\t\t\t" + conn.getURL());
//            if(bEnableLog) log.info(core + "<index update process> Response message\t\t\t" + conn.getResponseMessage());
//            if(bEnableLog) log.info(core + "<index update process> Response code\t\t\t" + conn.getResponseCode());
...
//	    		ByteArrayOutputStream outstream = new ByteArrayOutputStream(1024 * 4);
//	    		byte[] buffer = new byte[1024 * 4];
//	    		int len = -1;
//	    		while((len=instream.read(buffer, 0, 1024 * 4)) > 0) {
//	    			outstream.write(buffer, 0, len);
//	    		}
//	    		outputData = new String(outstream.toString().trim().getBytes(), "utf-8");
//	    		instream.close();
//	    		outstream.close();
//			}catch(Exception e) {
//				e.printStackTrace();
//				outputData = "~获取HTTP输出数据错误!" + e.toString();
//			}
//            taskLog.setTask_response(outputData); //conn.getResponseMessage()
//            if(conn.getResponseCode() == 200){
//                bResult = true;
//            }
//            conn.disconnect();
//            if(bEnableLog) log.info(core + "<index update process> Disconnected from server\t\t");
//        }catch(Exception e) {
//        	taskLog.setTask_response("~" + e.getMessage());
//        	if(bEnableLog) log.error("Failed to send HTTP POST", e);
//        }
//        return bResult;
//    }
	...
	/**
   	 * 判断字符串是否为数字
   	 * @param str 空字符串默认是数字
   	 * @param rule 具体的正则表达式 [0-9]* 判断正整数  -?[0-9]+.?[0-9]+ 所有数字
   	 * @return
   	 */
...
//        Map data = JSONUtils.fromJson("{\"msgtype\":\"request\", \"business\":\"cmdCtrl\", \"body\":{\"neid\":\"900000012104318\", \"inspectSessionId\":\"82327\", \"timeout\":\"\", \"items\":[{\"executive\":\"sqlUpdateScript\",\"ctrlColId\":\"241\",\"function\":\"null\",\"configneid\":\"\",\"param\":\"insert into AUTO_EXEC_TEST (M_ID, PARENT_M_ID, M_NAME, M_LEVEL, PATH, SORT_ID, SCRIPT_NAME, TABLE_NAME, FORM_ID, MAIN_TABLE_ID, SUB_REQUEST_ID, AUDIT_STATE, ADMIN_IDS, CI_INSTANCE_IDS, USE_STAFF_IDS, AUTHORIZ_IDS) values (M_ID_SEQ.NEXTVAL, '637', '数据库', '0', '-1/637/640', '2', '', 'DB_ACCOUNT_PORT_INFO11', '600004', '148', '', '', '1', '', '1,6571,6631', '')\",\"timeout\":\"30\"}]}}", Map.class);
//        List<Map<String, String>> datas = (List<Map<String, String>>)((Map)data.get("body")).get("items");
//        for(Map<String, String> m : datas) {
//        	System.err.println(m.get("ctrlColId"));	
//        }        
//   	
//        for (String token : tokenizeToStringArray("aaa   bbbb ccc", " ")) {
...
//   			FileInputStream fis = new FileInputStream("d://json.data");
...
//                    sbf.append((char)byteBuffer.get());
//                }
//                byteBuffer.clear();
...
//                fis.close();
//            }
//            //System.err.println(new String(sbf.toString().getBytes(), "iso-8859-1"));
//            Map data = JSONUtils.fromJson(sbf.toString(), Map.class);
//            System.err.println(((Map)data.get("FORM")).get("RECEIVER_OPINION_TIME"));
//   		}catch(Exception e){
//   			e.printStackTrace();
//   		}
//   		**/
//        System.err.println(StringUtils.getElementText(StringUtils.getElementBySinglePath("<root><id>123456</id></root>", "root#id")));   		
//   	}
