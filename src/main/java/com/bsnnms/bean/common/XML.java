...
public class XML
{
    ...
    public static String rsToXMLNoKey(ResultSet rs, XMLAction action)
        ...
        try
        {
            ...
            while (rs.next())
            {
                ...
                for (i = 1; i <= labels.length; i++)
                {
                    tagName = labels[i - 1];
                    value = action
                            .actionProcessRs(rs.getString(i), tagName, rs);
    ...
    public static String rsToXML(ResultSet rs, XMLAction action, int[] key,
            boolean isAddAttribute) throws SystemException, ApplicationException
    {
        StringBuffer xmlReturn = new StringBuffer("");
        int i = 0;
        try
        {
            ...
            while (rs.next())
            {
                ...
                for (i = 1; i <= labels.length; i++)
                {
                    ...
                    {
                        tagName = labels[i - 1];
                        value = action.actionProcessRs(rs.getString(i),
                                tagName, rs);
                        tagName = Encode(tagName);
                        value = Encode(value);
                        xmlReturn.append("<").append(tagName).append(">");
	...
	/**
	 * 将Map转换成Xml字符串
	 * @param xmlMap 需要转换成XML字符串的Map
	 * @return XML字符串
	 */
	public static String mapToXml(Map<String,String> xmlMap)
	{
		String template = "<{$key}>{$value}</{$key}>";
		...
		for (String key : keySet)
		{
			...
			attrs.put("key", key);
			attrs.put("value", xmlMap.get(key));
			sbXml.append(st.apply(attrs));
		}
